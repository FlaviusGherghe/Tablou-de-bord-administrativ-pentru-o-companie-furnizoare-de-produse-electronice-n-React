...
const handleDelete = async (id) => {
  try {
    await axios.delete(`http://localhost:8800/comenzi/${id}`);
...
useEffect(() => {
  const fetchAllRows = async () => {
    try {
      const res = await axios.get("http://localhost:8800/comenzi");
      setRows(res.data);
...
const handleClick = async (event) => {
  ...
  try {
    const res = await axios.post("http://localhost:8800/comenzi", {
      icon: file ? iconUrl : "",
      ...row,
    });
    window.location.reload()
  ...
  setInvoiceSubtotal(calculatedSubtotal);
    ...
    return (
      ...
      <TableRow key={row.id}>
        ...
        <TableCell align="center">{row.datalivrare}</TableCell>
        ...
        <TableCell align="center">
         <div className="butoane"><Link to={`/modificacomenzi/${row.id}`} style={{ color: "inherit", textDecoration: "none" }}>
    ...
    <h2 className="versiunea1">Adauga o comanda</h2>
     ...
     <label className="icon"><label htmlFor='file'><img  src={img32} alt="" className="icon" /></label></label>
       ...
       <input type="date" name="datalivrare" value={row.datalivrare} onChange={handleChange} placeholder="Data Livrare" />
